...
$("#btn_forward, #btn_back").bind('touchend', function(){
    $("#result").html ("stop");
    sendDirection("stop");
});
...
function sendDirection(direction) {
	$.getJSON('https://carpet.plumblem.com/set_car_direction?direction=' + direction, function(data) {		
	});
}
...
function loadA(){
	setTimeout(function(){
		...
		b.src='https://carpet.plumblem.com/camera?'+Date.now();
	}, 300);
}
...
function loadB(){
	setTimeout(function(){
		...
		a.src='https://carpet.plumblem.com/camera?'+Date.now();
	}, 300);
}
